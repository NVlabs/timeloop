...
from . import oaves_process_data
...
def RunOAVESPostProcessing(
    ...
    try:
        preprocessed_csv = output_dir / "timeloop-mapper.oaves.csv"
        oaves_csv = output_dir / "oaves.csv"
        logger.info(
            f"RunOAVESPostProcessing> process {preprocessed_csv} to {oaves_csv}"
        )
        oaves_process_data.process_data(
            preprocessed_csv,
            oaves_csv,
        ...
        raise Exception("RunOAVESPostProcessing Failed!")
...
def get_stats_files(output_dir, probs, pareto_optimal=True):
    if pareto_optimal:
        return [output_dir / prob.to_str() / "oaves.csv" for prob in probs]
    else:
        return [output_dir / prob.to_str() / "timeloop-mapper.oaves.csv" for prob in probs]
