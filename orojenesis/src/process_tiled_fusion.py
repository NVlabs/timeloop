        ...
        def process_chain(output_dir, chains, batch, scheme, cur_dir, output_prefix):
            ...
            for m_order in range(P_prob_log + 1):
                ...
                output_dir_ffmt = pathlib.Path(
                    f"{output_dir}/{arch_prefix}{model_name}/orojenesis_chain_2levels_outputs{batch}{spatial}_p{m_value}"
                )
                output_dir_allowk = pathlib.Path(
                    f"{output_dir}/{arch_prefix}{model_name}/orojenesis_chain_2levels_outputs{batch}{spatial}_allowk_p{m_value}"
                )
                output_dir_allown = pathlib.Path(
                    f"{output_dir}/{arch_prefix}{model_name}/orojenesis_chain_2levels_outputs{batch}{spatial}_allown_p{m_value}"
                )
                output_dir_allowkn = pathlib.Path(
                    f"{output_dir}/{arch_prefix}{model_name}/orojenesis_chain_2levels_outputs{batch}{spatial}_allowkn_p{m_value}"
                )
                output_dir_allownk = pathlib.Path(
                    f"{output_dir}/{arch_prefix}{model_name}/orojenesis_chain_2levels_outputs{batch}{spatial}_allownk_p{m_value}"
                )

                output_dir_allowk_flash = pathlib.Path(
                    f"{output_dir}/{arch_prefix}{model_name}/orojenesis_chain_2levels_outputs{batch}{spatial}_allowk_flash_p{m_value}"
                )
                output_dir_allown_flash = pathlib.Path(
                    f"{output_dir}/{arch_prefix}{model_name}/orojenesis_chain_2levels_outputs{batch}{spatial}_allown_flash_p{m_value}"
                )
                        ...
                        for prob_idx, prob in enumerate(pair):
                            if relax_io == "_relax_io":  # flat
                                if prob_idx == 0 and prob_idx == len(pair) - 1:
                                    ...
                                    orojenesis_csv = (
                                        output_dir_ffmt / layers_dict[prob] / orojenesis_file
                                    )
                            ...
                            if len(pair) == 1:
                                ...
                                orojenesis_csv = (
                                    output_dir_opt / layers_dict_nc[bmm_prob] / orojenesis_file
                                )
